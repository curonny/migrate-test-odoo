...
class AccountTourUploadBill(models.TransientModel):
    ...
    sample_bill_preview = fields.Binary(
        readonly=True, compute="_compute_sample_bill_image"
    )
    ...
    def _compute_sample_bill_image(self):
        ...
        try:
            ...
            self.sample_bill_preview = (
                base64.b64encode(open(path, "rb").read()) if path else False
            )
    ...
    def apply(self):
        ...
        if self.selection == "upload":
            ...
            bill_action = purchase_journal.with_context(
                default_journal_id=purchase_journal.id, default_move_type="in_invoice"
