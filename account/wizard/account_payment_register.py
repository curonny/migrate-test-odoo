...
class AccountPaymentRegister(models.TransientModel):
    ...
    def _compute_payment_method_id(self):
        for wizard in self:
            available_payment_methods = wizard._get_batch_available_payment_methods(
                wizard.journal_id, wizard.payment_type
            )
    ...
    def _create_payment_vals_from_batch(self, batch_result):
        ...
        if batch_values["payment_type"] != payment_method.payment_type:
            payment_method = self._get_batch_available_payment_methods(
                self.journal_id, batch_values["payment_type"]
            )[:1]
