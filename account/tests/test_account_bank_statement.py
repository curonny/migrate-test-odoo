...
class TestAccountBankStatementLine(TestAccountBankStatementCommon):
    ...
    def test_reconciliation_statement_line_with_generated_payments(self):
        ...
        test_line_2 = test_move.line_ids.filtered(
            lambda line: line.name == "test line 2"
        )

        statement_line = self.statement_line
        StatementLine_prepare_reconciliation = type(
            statement_line
        )._prepare_reconciliation

        def _prepare_reconciliation(
            self, lines_vals_list, create_payment_for_invoice=False
        ):
            if self == statement_line:
                create_payment_for_invoice = True
            return StatementLine_prepare_reconciliation(
                self, lines_vals_list, create_payment_for_invoice
            )

        with patch.object(
            type(statement_line), "_prepare_reconciliation", _prepare_reconciliation
        ):
            self.statement_line.reconcile(
                [
        ...
        self.assertRecordValues(
            test_line_1.matched_credit_ids.credit_move_id.payment_id.line_ids.sorted(
                "balance"
            ),
            [
                {
        ...
        self.assertRecordValues(
            test_line_2.matched_credit_ids.credit_move_id.payment_id.line_ids.sorted(
                "balance"
            ),
            [
                {
    ...
    def test_reconciliation_line_with_no_partner(self):
        ...
        receivable_account = self.company_data["default_account_receivable"]
        outstanding_account = self.company_data["default_journal_bank"][
