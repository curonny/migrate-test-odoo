...
class TestReconciliationMatchingRules(AccountTestInvoicingCommon):
    ...
    def _create_invoice_line(
        ...
        invoice_form = Form(
            cls.env["account.move"].with_context(
                default_move_type=type,
                default_invoice_date="2019-09-01",
                default_date="2019-09-01",
            )
        )
    ...
    def test_reverted_move_matching(self):
        ...
        payment_bnk_line = move.line_ids.filtered(
            lambda l: l.account_id == self.bank_journal.payment_credit_account_id
        )
    ...
    def test_inv_matching_with_write_off(self):
        ...
        to_compare = {
            key: full_write_off_dict[key] for key in expected_write_off.keys()
        }
    ...
    def test_avoid_amount_matching_bypass(self):
        ...
        self.env["ir.config_parameter"].set_param(
            "account.disable_rec_models_bypass", "1"
        )
        self.rule_1.match_total_amount_param = 90
        ...
        self._check_statement_matching(
            self.rule_1 + second_inv_matching_rule,
            {
                self.bank_line_1.id: {
                    "aml_ids": [self.invoice_line_1.id],
                    "model": self.rule_1,
                ...
                self.bank_line_2.id: {
                    "aml_ids": [self.invoice_line_2.id],
                    "model": second_inv_matching_rule,
                    "partner": self.bank_line_2.partner_id,
                },
            },
            statements=self.bank_st,
