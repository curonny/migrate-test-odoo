...
class AccountMove(models.Model):
    ...
    def _onchange_partner_id(self):
        ...
        if self.is_sale_document(include_receipts=True) and self.partner_id:
            ...
            self.narration = self.company_id.with_context(
    ...
    def update_lines_tax_exigibility(self):
        if all(
            ...
            tax_lines_caba = self.line_ids.filtered(
            ...
            base_lines_caba = self.line_ids.filtered(
                lambda x: any(
                    ...
                    or (
                        ...
                        and "on_payment"
                        in tax.mapped("children_tax_ids.tax_exigibility")
                    )
    ...
    def _onchange_name_warning(self):
        ...
        if self.name and self.name != "/" and origin_name and origin_name != "/":
            ...
            if format != origin_format or dict(format_values, seq=0) != dict(
                ...
                format_values["formatted_seq"] = "{seq:0{seq_length}d}".format(
                    **format_values
                )
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        def invert_tags_if_needed(repartition_line, tags):
            ...
            tags_need_inversion = self._tax_tags_need_inversion(
                self,
                (
                    (tax_type == "purchase" and line_vals["credit"] > 0)
                    or (tax_type == "sale" and line_vals["debit"] > 0)
                ),
                tax_type,
            )
            if tags_need_inversion:
        ...
        for line_command in move_vals.get("line_ids", []):
            ...
            if line_vals.get("tax_repartition_line_id"):
                ...
                for refund_repartition_line in refund_repartition_lines:
                    tag_ids += invert_tags_if_needed(
                        refund_repartition_line, refund_repartition_line.tag_ids
                    ).ids
        ...
        for move in to_post:
            ...
            if (
                ...
                move.date = move._get_accounting_date(
                    move.invoice_date or move.date, True
                )
                move.with_context(check_move_validity=False)._onchange_currency()
...
class AccountMoveLine(models.Model):
        ...
        for record in self:
            ...
            for tag in record.tax_tag_ids:
                ...
                if record.move_id.tax_cash_basis_rec_id:
                    ...
                    type_multiplicator = (
                        record.journal_id.type == "sale"
                        and self._get_not_entry_condition(record)
                        and -1
                        or 1
        ...
        if not self.env.context.get("tracking_disable", False):
            ...
            for move_id, modified_lines in move_initial_values.items():
                ...
                for line in self.filtered(lambda l: l.move_id.id == move_id):
                    changes, tracking_value_ids = line._mail_track(
                        ref_fields, modified_lines
                    )  # Return a tuple like (changed field, ORM command)
                    if tracking_value_ids:
                        for value in tracking_value_ids:
                            selected_field = value[
                                2
                            ]  # Get the last element of the tuple in the list of ORM command. (changed, [(0, 0, THIS)])
                        ...
                        for change in changes:
                            field_name = line._fields[
                                change
                            ].string  # Get the field name
                            tmp_move[move_id].append(
                                {
                                    "line_id": line.id,
                                    "error": True,
            ...
            for move in self.mapped("move_id"):
                ...
                if len(fields) > 0:
                    ...
                    move.message_post(
                        body=msg
                    )  # Write for each concerned move the message in the chatter
    ...
    def unlink(self):
        ...
        if not self.env.context.get("force_delete", False) and any(
            m.state == "posted" for m in moves
        ):
    ...
    def _get_formated_values(self, tracked_field):
        if tracked_field.get("field_type") in ("date", "datetime"):
            return {
                "old_value": format_date(
                    self.env,
                    fields.Datetime.from_string(
                        tracked_field.get("old_value_datetime")
                    ),
                ),
                "new_value": format_date(
                    self.env,
                    fields.Datetime.from_string(
                        tracked_field.get("new_value_datetime")
                    ),
                ),
            }
            ...
            return {
                "old_value": [
                    val for key, val in tracked_field.items() if "old_value" in key
                ][
                    0
                ],  # Get the first element because we create a list like ['Elem']
                "new_value": [
                    val for key, val in tracked_field.items() if "new_value" in key
                ][
                    0
                ],  # Get the first element because we create a list like ['Elem']
            }
    ...
    def _get_tracking_field_string(self, fields):
        ...
        for field in fields:
            redirect_link = (
                "<a href=# data-oe-model=account.move.line data-oe-id=%d>#%d</a>"
                % (field["line_id"], field["line_id"])
            )  # Account move line link
            if field.get("error", False):
                msg += "<li>%s: %s</li>" % (
                    field["field_error"],
                    _(
                        "A modification has been operated on the line %s.",
                        redirect_link,
                    ),
                )
            else:
    ...
    def _create_exchange_difference_move(self):
        ...
        def _add_cash_basis_lines_to_exchange_difference_vals(
            ...
            for move in lines.move_id:
                ...
                for line in move_values["to_process_lines"].filtered(
                    ...
                    vals = {
                        ...
                        "tax_tag_ids": [
                            (
                                6,
                                0,
                                line._convert_tags_for_cash_basis(line.tax_tag_ids).ids,
                            )
                        ],
                        "debit": line.debit,
                        "credit": line.credit,
        ...
        if exchange_diff_move_vals["line_ids"]:
            ...
            exchange_move.line_ids.write(
                {"tax_exigible": True}
            )  # Enforce exigibility in case some cash basis adjustments were made in this exchange difference
        else:
            return None
    ...
    def _revert_signed_tags(self, tags):
        ...
        for tag in tags:
            if tag.tax_report_line_ids:
                ...
                new_tag = tag.tax_report_line_ids[0].tag_ids.filtered(
                    lambda x: x.tax_negate != tag.tax_negate
                )
                rslt += new_tag
            else:
