...
class AccountPayment(models.Model):
        ...
        for pay in self:
            if pay.payment_type == "inbound":
                pay.available_payment_method_ids = (
                    pay.journal_id.inbound_payment_method_ids
                )
            else:
                pay.available_payment_method_ids = (
                    pay.journal_id.outbound_payment_method_ids
                )
    ...
    def _compute_stat_buttons_from_reconciliation(self):
        ...
        self.env["account.journal"].flush(
            fnames=["payment_debit_account_id", "payment_credit_account_id"]
        )
        self.env["account.payment"].flush(fnames=["move_id"])
        self.env["account.move"].flush(
            fnames=["move_type", "payment_id", "statement_line_id", "journal_id"]
        )
    ...
    def name_get(self):
        return [
            (payment.id, payment.move_id.name or _("Draft Payment")) for payment in self
        ]
