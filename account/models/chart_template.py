...
class AccountChartTemplate(models.Model):
    ...
    def existing_accounting(self, company_id):
        ...
        if (
            ...
            .sudo()
            .search([("company_id", "=", company_id.id), ("name", "!=", "/")], limit=1)
        ):
            return True
        return False
                ...
                todo.create(
                    {
                        ...
                        "name": _("Choose Accounting Template"),
                    }
                )
        return True
        ...
        for key, value in generated_tax_res["account_dict"]["account.tax"].items():
            if value["cash_basis_transition_account_id"]:
                AccountTaxObj.browse(key).write(
                    {
                        "cash_basis_transition_account_id": account_ref.get(
                            value["cash_basis_transition_account_id"], False
                        ),
                    }
                )
...
class AccountTaxTemplate(models.Model):
    ...
    def _generate_tax(
        ...
        tax_template_to_tax = {
            template.id: tax.id for (template, tax) in existing_template_to_tax
        }
        ...
        while templates_todo:
            ...
            for template in templates:
                if all(
                    child.id in tax_template_to_tax
                    for child in template.children_tax_ids
                ):
                    if accounts_exist:
