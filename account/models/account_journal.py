...
class AccountJournal(models.Model):
    ...
    payment_debit_account_id = fields.Many2one(
        ...
        domain=lambda self: "[('deprecated', '=', False), ('company_id', '=', company_id), \
                             ('user_type_id.type', 'not in', ('receivable', 'payable')), \
                             '|', ('user_type_id', '=', %s), ('id', '=', default_account_id)]"
        ...
        for journal in self:
            if journal.type in ("bank", "cash"):
                journal.outbound_payment_method_ids = (
                    self._default_outbound_payment_methods()
                )
            else:
        ...
        for journal in self:
            if journal.type in ("bank", "cash"):
                journal.inbound_payment_method_ids = (
                    self._default_inbound_payment_methods()
                )
            else:
    ...
    def _fill_missing_values(self, vals):
        ...
        if journal_type in ("bank", "cash"):
            ...
            if not has_payment_accounts:
                ...
                vals["payment_credit_account_id"] = (
                    self.env["account.account"]
                    .create(
                        {
