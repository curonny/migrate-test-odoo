...
class AccountEdiFormat(models.Model):
    ...
    def _export_facturx(self, invoice):
        ...
        for tax_detail in aggregated_taxes_details.values():
            tax_detail["tax_base_amount"] = (
                balance_multiplicator * tax_detail["base_amount_currency"]
            )
            tax_detail["tax_amount"] = (
                balance_multiplicator * tax_detail["tax_amount_currency"]
            )
            ...
            tax_detail["line"] = invoice.line_ids.filtered(
                lambda l: l.tax_line_id and l.tax_line_id.amount == tax_detail["amount"]
            )[:1]

        # Invoice lines.
        for i, line in enumerate(
            invoice.invoice_line_ids.filtered(lambda l: not l.display_type)
        ):
            ...
            line_template_values = {
                ...
                "price_discount_unit": (gross_price_subtotal - line.price_subtotal)
                / line.quantity
                if line.quantity
                else 0.0,
                "unece_uom_code": line.product_id.product_tmpl_id.uom_id._get_unece_code(),
                "gross_price_total_unit": line._prepare_edi_vals_to_export()[
            ...
            for tax_res in taxes_res["taxes"]:
                ...
                tax_category_code = tax._get_unece_category_code(
                    invoice.commercial_partner_id, invoice.company_id
                )
        ...
        with Form(
            ...
            invoice_form.partner_id = self_ctx._retrieve_partner(
                name=self._find_value(
                    "//ram:" + partner_type + "/ram:Name", tree, namespaces=tree.nsmap
                ),
                mail=self._find_value(
                    "//ram:" + partner_type + "//ram:URIID[@schemeID='SMTP']",
                    tree,
                    namespaces=tree.nsmap,
                ),
                vat=self._find_value(
                    "//ram:" + partner_type + "/ram:SpecifiedTaxRegistration/ram:ID",
                    tree,
                    namespaces=tree.nsmap,
                ),
            )
