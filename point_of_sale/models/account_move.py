...
class AccountMove(models.Model):
    ...
    def _tax_tags_need_inversion(self, move, is_refund, tax_type):
        ...
        if move.move_type == "entry" and move._origin.id:
            orders_count = self.env["pos.order"].search_count(
                [("account_move", "=", move._origin.id)]
            )
            sessions_count = self.env["pos.session"].search_count(
...
class AccountMoveLine(models.Model):
    ...
    def _get_not_entry_condition(self, aml):
        ...
        pos_orders_count = self.env["pos.order"].search_count(
            [("account_move", "=", aml.move_id.id)]
        )
