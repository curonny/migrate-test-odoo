...
class PosConfig(models.Model):
    ...
    def _compute_customer_html(self):
        for config in self:
            config.customer_facing_display_html = self.env["ir.qweb"]._render(
    ...
    def _check_rounding_method_strategy(self):
        if self.cash_rounding and self.rounding_method.strategy != "add_invoice_line":
            raise ValidationError(
                _("Cash rounding strategy must be: 'Add a rounding line'")
            )
    ...
    def _check_company_invoice_journal(self):
        if (
            ...
            and self.invoice_journal_id.company_id.id != self.company_id.id
        ):
            raise ValidationError(
                _(
    ...
    def _check_company_payment(self):
        if self.env["pos.payment.method"].search_count(
            [
                ("id", "in", self.payment_method_ids.ids),
    ...
    def _check_companies(self):
        if any(
            self.available_pricelist_ids.mapped(
                lambda pl: pl.company_id.id not in (False, self.company_id.id)
            )
        ):
            raise ValidationError(
                _(
