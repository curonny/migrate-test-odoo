...
class TestPoSWithFiscalPosition(TestPoSCommon):
    ...
    def test_01_no_invoice_fpos(self):
        ...
        for balance, amount in zip(
            sorted(sale_account_lines.mapped("balance")), sorted(lines_balance)
        ):
            self.assertAlmostEqual(balance, amount)

        other_sale_account_lines = session_move.line_ids.filtered(
        ...
        for balance, amount in zip(
            sorted(other_sale_account_lines.mapped("balance")), sorted(lines_balance)
        ):
            self.assertAlmostEqual(balance, amount)
        ...
        self.assertAlmostEqual(
            len(manually_calculated_taxes), len(tax_lines.mapped("balance"))
        )
        for t1, t2 in zip(
    ...
    def test_03_invoiced_fpos(self):
        ...
        invoiced_order_1 = self.pos_session.order_ids.filtered(
            lambda order: uid1 in order.pos_reference
        )
        invoiced_order_2 = self.pos_session.order_ids.filtered(
            lambda order: uid2 in order.pos_reference
        )
        ...
        self.assertFalse(
            sale_account_lines, msg="There should be no self.sale_account lines."
        )
        ...
        self.assertAlmostEqual(
            ...
            -691.06,
            msg="That is not the correct receivable line balance.",
        )

        other_receivable_line = session_move.line_ids.filtered(
            lambda line: line.account_id == self.other_receivable_account
        )
        self.assertAlmostEqual(
            other_receivable_line.balance,
            -265.75,
            msg="That is not the correct other receivable line balance.",
        )
