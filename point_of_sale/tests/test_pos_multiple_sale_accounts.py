...
class TestPoSMultipleSaleAccounts(TestPoSCommon):
    ...
    def test_01_check_product_properties(self):
        self.assertEqual(
            self.product2.property_account_income_id,
            self.other_sale_account,
            "Income account for the product2 should be the other sale account.",
        )
        self.assertFalse(
            self.product1.property_account_income_id,
            msg="Income account for product1 should not be set.",
        )
        self.assertFalse(
            self.product3.property_account_income_id,
            msg="Income account for product3 should not be set.",
        )
        self.assertEqual(
            self.product1.categ_id.property_account_income_categ_id, self.sale_account
        )
        self.assertEqual(
            self.product3.categ_id.property_account_income_categ_id, self.sale_account
        )
    ...
    def test_02_orders_without_invoice(self):
        ...
        sale_account_lines = session_move.line_ids.filtered(
            lambda line: line.account_id == self.sale_account
        )
        for balance, amount in zip(
            sorted(sale_account_lines.mapped("balance")), sorted([-164.85, -422.59])
        ):
            self.assertAlmostEqual(balance, amount)

        other_sale_account_line = session_move.line_ids.filtered(
            lambda line: line.account_id == self.other_sale_account
        )
    ...
    def test_03_orders_with_invoice(self):
        ...
        for balance, amount in zip(
            sorted(sale_account_lines.mapped("balance")), sorted([-164.85, -281.73])
        ):
            self.assertAlmostEqual(balance, amount)
