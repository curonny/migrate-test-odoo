...
class TestPoSBasicConfig(TestPoSCommon):
    ...
    def test_orders_no_invoiced(self):
        ...
        orders.append(
            self.create_ui_order_data([(self.product2, 7), (self.product3, 1)])
        )
        orders.append(
        ...
        sales_line = session_move.line_ids.filtered(
    ...
    def test_orders_with_invoiced(self):
        ...
        self.assertAlmostEqual(
            ...
            130,
            msg="Amount total should be 130. Product is untaxed.",
        )
        ...
        self.assertAlmostEqual(
            sales_line.balance,
            -(orders_total - invoice.amount_total),
            msg="Sales line should be total order minus invoiced order.",
        )
        ...
        self.assertAlmostEqual(
            pos_receivable_line_bank.balance,
            540.0,
            msg="Bank receivable should be equal to the total bank payments.",
        )
        ...
        self.assertAlmostEqual(
            pos_receivable_line_cash.balance,
            150.0,
            msg="Cash receivable should be equal to the total cash payments.",
        )
        ...
        self.assertEqual(
            receivable_line.full_reconcile_id, invoice_receivable_line.full_reconcile_id
        )
    ...
    def test_orders_with_zero_valued_invoiced(self):
        ...
        receivable_line = self.pos_session.move_id.line_ids.filtered(
            lambda line: line.account_id == self.receivable_account
        )
    ...
    def test_return_order(self):
        ...
        orders.append(
            self.create_ui_order_data(
                [(self.product1, 1), (self.product2, 5)], payments=[(self.bank_pm, 110)]
            )
        )
        orders.append(
            self.create_ui_order_data(
        ...
        self.assertEqual(
            len(sale_lines), 2, msg="There should be lines for both sales and refund."
        )
        ...
        self.assertFalse(
            ...
            "There should be no receivable cash line because both the order and return order are paid with cash - they cancelled.",
        )
    ...
    def test_split_cash_payments(self):
        ...
        bank_receivable_lines = account_move.line_ids.filtered(
            lambda line: self.bank_pm.name in line.name
        )
        self.assertEqual(
            len(bank_receivable_lines),
            1,
            msg="Bank receivable lines should only have one line because it's supposed to be combined.",
        )
        self.assertAlmostEqual(
            bank_receivable_lines.balance,
            300.0,
            msg="Bank receivable should be equal to the total bank payments.",
        )

        cash_receivable_lines = account_move.line_ids.filtered(
            lambda line: self.cash_split_pm.name in line.name
        )
        self.assertEqual(
            len(cash_receivable_lines),
            3,
            msg="There should be a number of cash receivable lines because the cash_pm is `split_transactions`.",
        )
        self.assertAlmostEqual(
            sum(cash_receivable_lines.mapped("balance")),
            290,
            msg="Total cash receivable balance should be equal to the total cash payments.",
        )

        for line in cash_receivable_lines:
            self.assertTrue(
                line.full_reconcile_id,
                msg="Each cash receivable line should be fully-reconciled.",
            )
    ...
    def test_correct_partner_on_invoice_receivables(self):
        ...
        orders.append(
            self.create_ui_order_data(
                [(self.product1, 10)],
                payments=[(self.cash_split_pm, 100)],
                customer=self.customer,
                is_invoiced=True,
                uid="00100-010-0003",
            )
        ...
        orders.append(
            self.create_ui_order_data(
                [(self.product1, 10)],
                payments=[(self.bank_split_pm, 100)],
                customer=self.customer,
                is_invoiced=True,
                uid="00100-010-0004",
            )
        ...
        customer_pos_receivable_bank = self.pos_session.move_id.line_ids.filtered(
            lambda line: line.partner_id == self.customer
            and "Split (Bank) PM" in line.name
        )
        self.assertEqual(
            len(customer_pos_receivable_bank),
            2,
            msg="there are 2 bank split payments from self.customer",
        )
        self.assertEqual(
            bool(customer_pos_receivable_bank.full_reconcile_id),
            False,
            msg="the pos (bank) receivable lines shouldn't be reconciled",
        )

        # self.customer's cash split payments
        customer_pos_receivable_cash = self.pos_session.move_id.line_ids.filtered(
            lambda line: line.partner_id == self.customer
            and "Split (Cash) PM" in line.name
        )
        self.assertEqual(
            len(customer_pos_receivable_cash),
            2,
            msg="there are 2 cash split payments from self.customer",
        )
        self.assertEqual(
            bool(customer_pos_receivable_cash.full_reconcile_id),
            True,
            msg="cash pos (cash) receivable lines should be reconciled",
        )

        # self.customer's invoice receivable counterpart
        customer_invoice_receivable_counterpart = (
            self.pos_session.move_id.line_ids.filtered(
                lambda line: line.partner_id == self.customer
                and "From invoiced orders" in line.name
            )
        )
        self.assertEqual(
            len(customer_invoice_receivable_counterpart),
            1,
            msg="there should one aggregated invoice receivable counterpart for self.customer",
        )
        self.assertEqual(
            bool(customer_invoice_receivable_counterpart.full_reconcile_id),
            True,
            msg="the aggregated receivable for self.customer should be reconciled",
        )
        self.assertEqual(
            customer_invoice_receivable_counterpart.balance,
            -500,
            msg="aggregated balance should be -500",
        )
        ...
        other_customer_invoice_receivable_counterpart = (
            self.pos_session.move_id.line_ids.filtered(
        ...
        self.assertEqual(
            len(other_customer_invoice_receivable_counterpart),
            1,
            msg="there should one aggregated invoice receivable counterpart for self.other_customer",
        )
        self.assertEqual(
            bool(other_customer_invoice_receivable_counterpart.full_reconcile_id),
            True,
            msg="the aggregated receivable for self.other_customer should be reconciled",
        )
        self.assertEqual(
            other_customer_invoice_receivable_counterpart.balance,
            -200,
            msg="aggregated balance should be -200",
        )
            ...
            [
                {
                    ...
                    "cashbox_lines_ids": [
                        (0, 0, {"number": 1, "coin_value": amount_paid})
                    ],
                    "is_a_template": False,
                }
            ]
                ...
                [
                    {
                        ...
                        "cashbox_lines_ids": [
                            (0, 0, {"number": 1, "coin_value": pos_data["amount_paid"]})
                        ],
                        "is_a_template": False,
                    }
                ]
    ...
    def test_refund_customer_reconcile(self):
        ...
        self.assertEqual(
            len(sale_lines), 0, msg="There should be no lines as all is invoiced."
        )
        self.assertEqual(
            sum(
