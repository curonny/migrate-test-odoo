...
class TestPoSOtherCurrencyConfig(TestPoSCommon):
    ...
    def test_02_orders_without_invoice(self):
        ...
        orders.append(
            self.create_ui_order_data([(self.product1, 5), (self.product2, 5)])
        )
        orders.append(
            self.create_ui_order_data(
        ...
        sale_account_line = session_move.line_ids.filtered(
        ...
        pos_receivable_line_bank = session_move.line_ids.filtered(
            lambda line: line.account_id == self.pos_receivable_account
            and self.bank_pm.name in line.name
        )
    ...
    def test_03_orders_with_invoice(self):
        ...
        invoice_receivable_line = session_move.line_ids.filtered(
    ...
    def test_04_anglo_saxon_products(self):
        ...
        orders = []
        orders.append(
            self.create_ui_order_data([(self.product4, 7), (self.product5, 7)])
        )
        orders.append(
            self.create_ui_order_data(
                [(self.product5, 6), (self.product4, 6), (self.product6, 49)]
            )
        )
        orders.append(
            self.create_ui_order_data([(self.product5, 2), (self.product6, 13)])
        )
        ...
        self.assertEqual(
            len(session_account_move.line_ids),
            4,
            msg="There should exactly be 4 account move lines.",
        )

        sales_line = session_account_move.line_ids.filtered(
            lambda line: line.account_id == self.sale_account
        )
        self.assertAlmostEqual(
            sales_line.balance,
        ...
        receivable_line_cash = session_account_move.line_ids.filtered(
            lambda line: self.pos_receivable_account == line.account_id
            and self.cash_pm.name in line.name
        )
        self.assertAlmostEqual(
        ...
        expense_line = session_account_move.line_ids.filtered(
        ...
        output_line = session_account_move.line_ids.filtered(
    ...
    def test_05_tax_base_amount(self):
        ...
        self.assertEqual(
            len(session_account_move.line_ids),
            3,
            msg="There should exactly be 3 account move lines.",
        )
        ...
        tax_line = session_account_move.line_ids.filtered(
        ...
        self.assertAlmostEqual(
            tax_line.tax_base_amount, 49, msg="Value should be in company's currency."
        )
