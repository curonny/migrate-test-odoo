...
class TestPointOfSaleFlow(TestPointOfSaleCommon):
    ...
    def test_order_to_picking02(self):
        ...
        """
        tracked_product_w_lot, tracked_product_wo_lot, untracked_product = self.env[
            "product.product"
        ].create(
            [
                {
        ...
        self.env["stock.quant"]._update_available_quantity(
            tracked_product_w_lot, shelf1_location, qty, lot_id=lot
        )
        self.env["stock.quant"]._update_available_quantity(
            tracked_product_wo_lot, shelf1_location, qty
        )
        self.env["stock.quant"]._update_available_quantity(
        ...
        for i in range(qty):
                    ...
                    "lines": [
                        ...
                        (
                            ...
                            0,
                            {
                                ...
                                "qty": 1.0,
                                "tax_ids": [
                                    (6, 0, tracked_product_wo_lot.taxes_id.ids)
                                ],
                                "price_subtotal": untax,
                                "price_subtotal_incl": untax + atax,
            ...
            tracked_line_w_lot = pos_order.picking_ids.move_line_ids.filtered(
                lambda ml: ml.product_id.id == tracked_product_w_lot.id
            )
            tracked_line_wo_lot = pos_order.picking_ids.move_line_ids.filtered(
                lambda ml: ml.product_id.id == tracked_product_wo_lot.id
            )
            untracked_line = pos_order.picking_ids.move_line_ids - (
                tracked_line_w_lot | tracked_line_wo_lot
            )
            ...
            self.assertEqual(
                tracked_line_wo_lot.location_id,
                pos_order.config_id.picking_type_id.default_location_src_id,
            )
